...
class YandexOCRAsync:
    ...
    def split_pdf(self, input_path, input_name, pages_per_chunk=1):
        """
        ...
        try:
            ...
            for start_page in range(0, total_pages, pages_per_chunk):
                ...
                chunk_path = f"./{input_name}_{start_page}.pdf"
    ...
    async def recognize_pdf(self, file_path):
        """
    ...
    async def get_operation_result(self, operation_id, max_retries=10, delay=10):
        """
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(f"Ошибка при запросе результата {str(e)}")
    ...
    async def process_chunk(self, chunk_path, page_number, begin_page, semaphore):
        """
        Process single PDF chunk with semaphore for concurrency control.
        """
        async with semaphore:
            try:
                operation_id = await self.recognize_pdf(chunk_path)
                if operation_id is None:
                    raise Exception("Не удалось получить ID операции распознавания")
                
                ocr_result = await self.get_operation_result(operation_id)
                if ocr_result is None:
                    raise Exception("Не удалось получить результат распознавания")
                
                result = {
                    "page": page_number + 1 + begin_page,
    ...
    async def process_pdf(self, input_path, input_name, begin_page=0, max_concurrent=10):
        """
        Main method to process entire PDF with parallel chunk processing.
        """
        try:
            logger.info(f"Начало обработки PDF файла: {input_name}")
            logger.info(f"Максимальное количество параллельных запросов: {max_concurrent}")
            
            chunks = self.split_pdf(input_path, input_name)
            semaphore = asyncio.Semaphore(max_concurrent)
            
            tasks = [
                asyncio.create_task(self.process_chunk(chunk_path, page_number, begin_page, semaphore))
